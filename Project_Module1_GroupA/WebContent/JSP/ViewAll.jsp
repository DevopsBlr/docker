...
<form action="<%=request.getContextPath()%>/AccountController">
	<table  border="2" cellspacing="5" cellpadding="5" id="results">
		<tr>
			  <th scope="col">
 Customer Id
  <input type="text" id="txtPrjName" onkeyup="searchRows('results')" >
  </th>
			<th>Status</th>
			<th>SSN Id</th>
			<th>Customer Name</th>
	...
	<% for(Customer c:clist){ %>
		...
		<td><a href="<%=request.getContextPath()%>/BankManagementController?source=changestatus&customer=<%=c.getCustomerId()%>">Change Status</a></td>
		<%if(c.getStatus().equals("active")){%>
				<td><a href="<%=request.getContextPath()%>/AccountController?action=add&customerid=<%=c.getCustomerId()%>">Add</a></td>
		<%} %>
	...
	<p align="right"><input  type="button" onclick="refresh()" value="Refresh" ></p>
